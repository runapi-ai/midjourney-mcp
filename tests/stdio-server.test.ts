...
describe("midjourney stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "midjourney-mcp-test", version: "0.1.0" });
    ...
    for (const model of ["midjourney-edit-image","midjourney-image-to-video","midjourney-v8.1"]) {
